<template>
  <div class="scroll-page">
    ...
    <div class="container">
      ...
      <!-- DataTable affichant les places dépliées pour PFP3 -->
      <DataTable :value="expandedPFP3" class="p-datatable-sm custom-datatable" responsiveLayout="scroll">
        <!-- Colonne Institution -->
        ...
        <Column header="Praticien Formateur">
          <template #body="slotProps">
            ...
            <div v-else>
              <div v-if="slotProps.data['selectedPraticiensBA23PFP3-' + slotProps.data.seatIndex]">
                <span>{{ praticiensFormateurs[slotProps.data['selectedPraticiensBA23PFP3-' + slotProps.data.seatIndex]] }}</span>
        ...
        <Column header="Etudiant">
          <template #body="slotProps">
            <Dropdown v-model="slotProps.data.dyn[selectedKey(slotProps.data)]"
              :options="ba23StudentsOptions" optionLabel="fullName" optionValue="id"
        ...
        <Column header="Sélection Out">
          <template #body="slotProps">
            <Checkbox v-model="slotProps.data[`selectedActiveBA23PFP3-${slotProps.data.seatIndex}`]"
      ...
      <div class="recap mt-4 surface-card">
        <h3>Récapitulatif</h3>
        <ul>
          <li>Nombre d'étudiants BA23 : {{ ba23Students.length }}</li>
          ...
          <li>Nombre de places sélectionnées pour PFP3 BA23 : {{ totalSelectedOut }}</li>
...
export default {
  ...
  computed: {
    // Options pour le Dropdown des étudiants BA23
    ba23Students() {
      return this.users.filter(user =>
        user.Roles && user.Roles.BA23 === true
      );
    },
    ba23StudentsOptions() {
      let options = this.ba23Students
        .map(user => ({
    ...
    // Seules les places dont PFP3 est défini et >= 1 sont incluses.
    expandedPFP3() {
      const rows = [];
      this.sortedPlaces.forEach(place => {
        let count = parseInt(place.PFP3);
        if (!isNaN(count) && count >= 1) {
          ...
          for (let i = 1; i <= count; i++) {
            const keyEtudiant = `selectedEtudiantBA23PFP3-${i}`;
            const keyPraticien = `selectedPraticiensBA23PFP3-${i}`;
            ...
            rows.push({
              ...
              [`selectedActiveBA23PFP3-${i}`]: (place[`selectedActiveBA23PFP3-${i}`] !== undefined ? place[`selectedActiveBA23PFP3-${i}`] : false),
              dyn,        // pour étudiant
    ...
    totalUniquePlaces() {
      return this.places.filter(place => {
        const count = parseInt(place.PFP3);
        return !isNaN(count) && count >= 1;
      }).length;
    },
    totalSelectedOut() {
      return this.expandedPFP3.filter(row => row[`selectedActiveBA23PFP3-${row.seatIndex}`] === true).length;
    },
    totalWithStudentSelected() {
      return this.places.filter(place => {
        ...
        let found = false;
        for (let i = 1; i <= count; i++) {
          const key = `selectedEtudiantBA23PFP3-${i}`;
    ...
    totalWithPraticienSelected() {
      return this.places.filter(place => {
        ...
        let found = false;
        for (let i = 1; i <= count; i++) {
          const key = `selectedPraticiensBA23PFP3-${i}`;
    ...
    linesWithSelectedPraticien() {
      return this.expandedPFP3.filter(row => this.praticienSelected(row));
    },
    ...
    casparticuliers() {
      return this.users.filter(user => user.Roles && user.Roles.casparticulier === true && user.Roles.BA23 === true);
    },
    // Nombre global de places offertes (somme de PFP3)
    totalPlacesOffertes() {
      return this.places.reduce((total, place) => total + (parseInt(place.PFP3) || 0), 0);
    },
  ...
  methods: {
    ...
    selectedKey(row) {
      return `selectedEtudiantBA23PFP3-${row.seatIndex}`;
    },
    updateRemark(row) {
    ...
    selectedKeyPraticien(row) {
      return `selectedPraticiensBA23PFP3-${row.seatIndex}`;
    },
    ...
    hasSelectedPraticien(row) {
      const keyPlural = `selectedPraticiensBA23PFP3-${row.seatIndex}`;
      const keySingular = `selectedPraticienBA23PFP3-${row.seatIndex}`;
    ...
    updatePlaceStudent(place, newStudentId, seatIndex) {
      ...
      const propertyName = `selectedEtudiantBA23PFP3-${seatIndex}`;
    ...
    updatePraticienSelection(row, newId, seatIndex) {
      const propertyName = `selectedPraticiensBA23PFP3-${seatIndex}`;
      const placeRef = ref(db, `Places/${row.IdPlace}`);
    ...
    onSelectActiveChange(row) {
      const dynamicKey = `selectedActiveBA23PFP3-${row.seatIndex}`;
      const placeRef = ref(db, `Places/${row.IdPlace}`);
