      ...
      <div class="grid">
        ...
        >
          ...
          <div>
            ...
            {{getInstitutionNameById(place.IDPlace) }}</h6>
              <p class="m-2">
              Domaine : {{ place.NomPlace }}<br />
              Critères : {{ getValidCriterias(place).join(', ') }}<br />
              Praticien formateur : {{ getPraticienFormateurInfos(place) }}<br />
            </p>
              <a :href="'/institution/' + place.IDPlace" target="_blank" style="color: var(--text-primary); text-decoration: underline;">
                Accéder à la page de l'institution
              </a>
            <p class="m-2">
